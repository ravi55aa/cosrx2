...
//Dynamic banner [Try to add it has the Dynamic one];
import headerBackgroundImage from "@/assets/Product/Tooner/banner.webp"

const DynamicCat = () => {
  const [sunscreenProducts, setSunScreenProducts] = useState([]);
  const [filsunscreenProducts, setSunfilScreenProducts] = useState([...sunscreenProducts] || []);
  ...
  const productsPerPage = 3; // Here iam setting the NoOf Products per pagination
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        let currentCat = categoryss.find((ele)=>ele?.name == catName);
  ...
  // Initialize AOS and GSAP animations [Not need to study, just for the animation];
  useEffect(() => {
  ...
  // Handle all the filter not given the sorign methods here if possible chage it latter
  //---------------
  const handleFilter = () => {
  ...
  //This will take the multiple properties from state
  ...
  // Get the current category name for breadcrumbs
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <section
        ...
        style={ 
          { 
            backgroundImage: `url(${ 
            category.some((ele)=>ele?.name == catName)
            ?
            currentCat?.banner
            :
            ...
            {currentProducts.length > 0 ? (
              currentProducts.map((product) => (
                <motion.div
                  ...
                  data-aos="fade-up"
                  data-aos-delay={product._id * 100}
                >
