...
import {fetchEditAddress,editAddress} from "@/Services/User/Profile/Address.jsx" 
...
const AddAddress = () => {
    ...
    useEffect(() => {
      ...
      const fetchAddress = async () => {
        const response = await fetchEditAddress(id);
        if (!response) {
        ...
        if (isMounted) {
          setAddressData((prev) => ({
            ...
            pincode: response?.data?.address[0]?.pincode?.toString() || "",
          }));
        }
    ...
    const handleSubmit = useCallback(async(e)=>{
        ...
        const isValidated = validate_address(updatedAddress);

        if(Object.keys(isValidated).length > 0){
            toast.warn(Object.entries(isValidated)[0][1]);
            return;
        }
        const response = await editAddress(updatedAddress);
    ...
    const handleSelect=useCallback((place)=>{
        ...
        setAddressData((prevData)=>({...prevData,city:Name,state:State}));
        handleCancel();
    },[OTPModal.open]);
    ...
    axios.get(`https://api.postalpincode.in/pincode/${addressData.pincode}`)
        .then((res)=>{
