...
const AddOfferPage = () => {
    ...
    // Update Apply To when Offer Type changes
    useEffect(() => {
        setFormData((prev) => ({ ...prev, applyTo: "" })); // Reset Apply To when Offer Type changes
    }, [formData.offerType]);
    ...
    const handleFields = () => {
        if(!validate_fields_business(formData)){
            return false;
        }
    ...
    const handleOfferExist = useCallback(async () => {
        try {
        const response = await axiosBaseUrl.post("/admin/offers/check-exist", {
            offerName: formData.offerName,
        });
        const result = response.data;

        if (result.exists) {
            toast.info("Offer with this name already exists");
            return true;
        }
        return false;
        } catch (err) {
        console.error(err.message);
        toast.error("Error checking offer existence");
        return false;
        }
    }, [formData.offerName]);
    ...
    const handleSubmit = useCallback(
        ...
        // const offerExists = await handleOfferExist();
        // if (offerExists) {
        //     setSpinner(false);
        //     return;
        // }

        const addOffer =  await admin_addOffer_success(formData);
        ...
        [formData, handleOfferExist, navigate]
    );
    ...
    return (
        ...
        <div className="flex-1 p-4 sm:p-6 md:p-8">
            ...
            <h1 className="text-2xl sm:text-3xl font-semibold">Add Offer</h1>
            <Bars3Icon
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    Discount Percentage {formData?.discountAmountType === "Percentage" ? "(%)" : "(₹)"}:
                    </label>
                    <input
                    type="number"
                    name="discountAmount"
